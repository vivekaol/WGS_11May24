...
File: L1C_RadiolinkSetupRespProcValidation.c
...
{
	0/*edchFddInfoRsp_present*/,
	2,
	1,
	1,
	1,
	l1c_CoordTrchInfoResp,
	NULL/*&l1c_EdchFddInfoResp*/
};
...
static L1CAPI_Radiolink_Setup_Response l1c_RlSetupResp = 
{
	0/*rl_SetupResp_hsdschInfoResp_present*/,
	0,
	1,
	{
		L1_MSG_SUCCESS,
...
uint8_t validateRlSetupRespCodecOutput(L1CAPI_Radiolink_Setup_Response * pCodecResult,L1CAPI_Radiolink_Setup_Response *pExpectedResult)
{
	...
	for(i=0;i<pCodecResult->rlCount;i++){
		...
		if(pCodecResult->ptrRlInfoResp[i].referenceRlId != pExpectedResult->ptrRlInfoResp[i].referenceRlId)
			return 0;
		if(memcmp(pCodecResult->ptrRlInfoResp[i].ptrCoordTrchRsp,pExpectedResult->ptrRlInfoResp[i].ptrCoordTrchRsp,sizeof(L1CAPI_CoordTrchRespItem)*pCodecResult->ptrRlInfoResp[i].coordTrchCount))
			return 0;
	...
	if(pCodecResult->bit_mask & rl_SetupResp_hsdschInfoResp_present)
	{
