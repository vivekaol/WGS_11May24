...
------------------------------------------------------------------------------*/
void EDIntToBitsInternal (void* Destination, ED_EXLONG DestOffset, int Source, int Count)
{
...
------------------------------------------------------------------------------*/
int EDBitsToIntInternal (const void* Source, ED_EXLONG SourceOffset, int Count)
{
#ifndef ED_REVERSED_INTEGER
		int i, ret=0;
		for (i=0; i<Count; i++) {
...
------------------------------------------------------------------------------*/
void EDIntToBitsReversedInternal (void* Destination, ED_EXLONG DestOffset, int Source, int Count)
{
...
/*------------------------------------------------------------------------------
	...
	ED_REVERSED_INTEGER is defined.
------------------------------------------------------------------------------*/
int EDBitsToIntReversedInternal (const void* Source, ED_EXLONG SourceOffset, int Count)
{
#ifdef ED_REVERSED_INTEGER
		int i, ret=0;
		for (i=0; i<Count; i++) {
...
int EDEncodeDefiniteFormLength (void* Destination, ED_EXLONG Offset, unsigned ED_EXLONG Value)
{
	...
	/* Encode in ED_EXLONG form */
		...
		/* Encode the data */
		EDIntToBits (Destination, Offset+8, Value, 8 * Size);
...
int EDDecodeDefiniteFormLength (const void* Source, ED_EXLONG Offset, ED_EXLONG* Value)
{
	...
	/* Decode the value */
	(*Value) = (ED_EXLONG)EDBitsToInt (Source, Offset+8, FirstOctet*8);
...
------------------------------------------------------------------------------*/
void EDStandardLocatorSet (unsigned ED_EXLONG* edLocatorPtr, ED_EXLONG offsetInBits, ED_EXLONG lengthInBits)
{
...
#define POS_IEEE(bit) (1 << (bit & 7))
#define SRC_IEEE (*(((const unsigned char*) Source)+(SourceOffset >> 3)))
#define DST_IEEE (*(((unsigned char*)Destination)+(DestOffset >> 3)))

/*------------------------------------------------------------------------------
	This function copies 'Count' less significant bits of 'Source' char buffer
	from offset 'Offset'. Read integer is returned.
------------------------------------------------------------------------------*/
int EDBitsToInt_IEEE (const void* Source, ED_EXLONG SourceOffset, int Count)
{
	int i, ret=0;
	SourceOffset+=Count-1;
	for (i=0; i<Count; i++) {
		if ((SRC_IEEE & POS_IEEE(SourceOffset)) != 0) ret |= (1 << (Count-i-1));
		SourceOffset--;
	}
	return ret;
}
...
/*------------------------------------------------------------------------------
	...
	'Destination' ad offset 'Offset'.
------------------------------------------------------------------------------*/
void EDIntToBits_IEEE (void* Destination, ED_EXLONG DestOffset, int Source, int Count)
{
	int i;
	DestOffset+=Count-1;
	for (i=Count-1; i>=0; i--) {
		if ((Source & (1 << i)) != 0) DST_IEEE |= POS_IEEE(DestOffset);
		else DST_IEEE &= ~POS_IEEE(DestOffset);
		DestOffset--;
	}
}
...
void EDCopyBits_IEEE (void* Destination, ED_EXLONG DestOffset, const void* Source, ED_EXLONG SourceOffset, ED_EXLONG Length)
{
	...
	for (l=0; l<Length; l++) {
		if ((SRC_IEEE & POS_IEEE(SourceOffset)) != 0) DST_IEEE |= POS_IEEE(DestOffset);
		else DST_IEEE &= ~POS_IEEE(DestOffset);
		SourceOffset++;
		DestOffset++;
	}
